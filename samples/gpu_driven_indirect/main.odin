...
main :: proc() {
	...
	window, window_ok := app.create_window({
		width         = 1280,
		height        = 360,
		title         = "Ape D3D12 GPU-Driven Indirect",
		no_client_api = true,
	})
	if !window_ok {
	...
	ctx, gfx_ok := gfx.init({
		...
		vsync            = true,
		debug            = true,
		label            = "ape d3d12 gpu driven indirect",
	})
	if !gfx_ok {
	...
	args_shader_desc, args_shader_desc_ok := shader_assets.shader_desc(&args_package, .D3D12_DXIL, "gpu_driven_indirect_args shader")
	...
	// Render pipeline: triangle shader from samples/triangle.
	...
	triangle_shader_desc, triangle_shader_desc_ok := shader_assets.shader_desc(&triangle_package, .D3D12_DXIL, "triangle shader")
