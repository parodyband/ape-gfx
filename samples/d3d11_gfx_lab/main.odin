...
main :: proc() {
	...
	for !app.should_close(&window) {
		...
		swapchain_action.colors[0].clear_value = gfx.Color{r = 0.010, g = 0.012, b = 0.016, a = 1}

		if !gfx.begin_pass(&ctx, {label = "lab swapchain display pass", action = swapchain_action}) {
...
make_texture_vertices :: proc(render_width, render_height: i32) -> [4]Texture_Vertex {
	half_width, half_height := ape_sample.aspect_fit_half_extents(render_width, render_height, OFFSCREEN_SIZE, OFFSCREEN_SIZE, DISPLAY_HALF_EXTENT)
