...
//   - ID3D11Buffer chunk re-`Map(WRITE_DISCARD)` on reset.
...
main :: proc() {
	state := Sample_State{}
	ok := gfx_app.run({
		title      = "Ape D3D11 Transient Uniforms",
		gfx_label  = "ape d3d11 transient uniforms",
		backend    = .D3D11,
		vsync      = true,
		debug      = true,
	...
	if !ok {
		fmt.eprintln("d3d11_transient_uniforms: gfx_app.run failed")
	}
}
