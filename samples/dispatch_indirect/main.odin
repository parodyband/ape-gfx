...
main :: proc() {
	...
	window, window_ok := app.create_window({
		width         = 320,
		height        = 240,
		title         = "Ape D3D12 Dispatch Indirect",
		no_client_api = true,
	})
	if !window_ok {
	...
	ctx, gfx_ok := gfx.init({
		...
		vsync            = false,
		debug            = true,
		label            = "ape d3d12 dispatch indirect",
	})
	if !gfx_ok {
	...
	shader_desc, shader_desc_ok := shader_assets.shader_desc(&shader_package, .D3D12_DXIL, "dispatch_indirect_fill shader")
	if !shader_desc_ok {
	...
	fmt.println("D3D12 dispatch_indirect produced same result as direct dispatch")
	_ = AUTO_EXIT_FRAMES
}
