...
main :: proc() {
	...
	window, ok := app.create_window({
		width = 1280,
		height = 720,
		title = "Ape D3D12 Render To Texture",
		no_client_api = true,
	})
	if !ok {
	...
	ctx, gfx_ok := gfx.init({
		...
		vsync = true,
		debug = true,
		label = "ape d3d12 render to texture",
	})
	if !gfx_ok {
	...
	for !app.should_close(&window) {
		...
		if !gfx.begin_pass(&ctx, {
			...
			action = {colors = {0 = {clear_value = {r = 0.04, g = 0.045, b = 0.06, a = 1}}}},
		}) {
		...
		// Sampled before the resolve pass binds it as a sampled view. D3D12
