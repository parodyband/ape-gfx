...
Regenerate the raw and Markdown API docs with:

```powershell
.\tools\generate_api_docs.ps1
```

The generated docs are tracked so API drift is visible in normal diffs. `tools/test_api_docs_public_surface.ps1` also checks that backend internals stay out of the public `gfx` docs.
...
- renderer, material, camera, scene, or gameplay framework layers
- web or mobile backends
- Vulkan implementation work
- KTX2/Basis texture pipeline
- async shader reload
- hidden global graphics context
